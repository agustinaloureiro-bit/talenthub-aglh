...
const SYNC_ENGINE_VERSION = "2026-07-22.2";
...
function prepareConfigForSave(integrationId: string, config?: Record<string, unknown>) {
  ...
  if (integrationId === "yoiners") {
    const hasCredentials = hasUsernamePassword(next);
    const providedSession = ["yoinersAccessToken", "yoinersRefreshToken", "apiKey", "token", "accessToken", "sessionCookies", "browserStorageState"]
      .some((key) => key in next && cleanText(next[key]).length > 0);
    if (hasCredentials && !providedSession) {
...
async function syncIntegration(integrationId: string) {
  ...
  if (agent) {
    ...
    try {
      const defaultSyncTimeoutMs = integrationId === "yoiners" ? 180_000 : 70_000;
      scraperResult = await withTimeout(
        agent.sync(config),
        numberFromConfig(config.syncTimeoutMs, defaultSyncTimeoutMs),
