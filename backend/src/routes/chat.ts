...
chatRouter.post("/sessions", asyncHandler(async (req, res) => {
  const title = z.object({ title: z.string().min(1).default("Nueva conversación") }).parse(req.body).title;
...
chatRouter.post("/sessions/:id/messages", asyncHandler(async (req, res) => {
  ...
  const matches = await findCandidates(body.content, { activeOnly: true });
  const refs = matches.slice(0, 5).map((m) => m.id);
  const content = refs.length
    ? `Encontré ${matches.length} candidatos activos relacionados con tu consulta. Te muestro los primeros resultados ordenados por relevancia.`
    : "No encontré candidatos activos que coincidan con la consulta en la base actual.";
  const { rows } = await q(
  ...
  res.status(201).json({ data: rows[0], candidates: matches.slice(0, 5) });
}));
