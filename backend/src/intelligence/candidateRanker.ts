...
export function explainCandidateMatch(candidate: TalentCandidateResult, interpreted: InterpretedTalentQuery) {
  const haystack = `${candidate.fullName} ${candidate.currentRole ?? ""} ${candidate.city ?? ""} ${candidate.country ?? ""} ${(candidate.tags ?? []).join(" ")} ${candidate.seniority ?? ""}`.toLowerCase();
...
export function rerankCandidates(candidates: TalentCandidateResult[], interpreted: InterpretedTalentQuery) {
  return candidates
    .map((candidate) => {
      ...
      for (const role of interpreted.roles) if (haystack.includes(role.toLowerCase())) intelligenceBoost += 18;
      for (const skill of interpreted.skills) if (haystack.includes(skill.toLowerCase())) intelligenceBoost += 10;
      for (const language of interpreted.languages) if (haystack.includes(language.toLowerCase())) intelligenceBoost += 12;
